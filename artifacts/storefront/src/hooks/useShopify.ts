...
import { FEATURED_COLLECTIONS_QUERY, PRODUCTS_QUERY, NEWEST_PRODUCTS_QUERY, PRODUCT_QUERY, SEARCH_PRODUCTS_QUERY } from "@/lib/queries";
...
export function useCollectionProducts(handle: string, first: number = 24, sortKey: string = "BEST_SELLING", reverse: boolean = false) {
  return useQuery({
    queryKey: ["collectionProducts", handle, first, sortKey, reverse],
    queryFn: async () => {
      const data = await shopifyFetch(PRODUCTS_QUERY, { handle, first, sortKey, reverse });
      return data.collection?.products.edges.map((e: any) => e.node) as Product[] || [];
    },
    enabled: !!handle,
  });
}

export function useNewestProducts(first: number = 8) {
  return useQuery({
...
export function useSearchProducts(query: string, first: number = 24) {
  return useQuery({
